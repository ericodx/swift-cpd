...
struct YamlConfigurationParserTests {
    ...
    @Test("Given inline empty enabledCloneTypes array, when parsing, then returns empty array")
    func inlineEmptyEnabledCloneTypesArray() throws {
        let config = try parser.parse("enabledCloneTypes: []")

        #expect(config.enabledCloneTypes == [])
    }

    @Test("Given single-char quoted value, when parsing, then returns it as-is")
    func singleCharQuotedValue() throws {
