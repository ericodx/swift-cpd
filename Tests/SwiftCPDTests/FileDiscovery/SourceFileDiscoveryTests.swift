...
struct SourceFileDiscoveryTests {
    ...
    @Test("Given unreadable directory, when finding, then returns empty")
    func unreadableDirectoryReturnsEmpty() throws {
        let tempDir = createTempDirectory(prefix: "SourceFileDiscovery-noperm")
        defer {
    ...
    @Test("Given non-source file path, when finding, then ignores it")
    func nonSourceFilePathIsIgnored() throws {
        let tempDir = createTempDirectory(prefix: "SourceFileDiscovery")
        defer { removeTempDirectory(tempDir) }

        let filePath = tempDir + "/README.txt"
        createFile(at: filePath)
