...
struct SemanticNormalizerTests {
    ...
    @Test("Given binding with non-literal initializer, when normalizing, then produces assignment but no literalValue")
    func bindingWithNonLiteralInitializer() {
        let source = """
            func process(_ items: [Int]) {
    ...
    @Test("Given integer literal as function call argument, when normalizing, then produces literalValue node")
    func integerLiteralAsFunctionArgument() {
        let source = """
            func process() {
                print(42)
            }
