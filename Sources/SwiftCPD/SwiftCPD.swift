...
extension SwiftCPD {
    ...
    private static func handleInit() -> ExitCode {
        ...
        let discoveredPaths = SourcePathDiscovery().discover()
        let pathsYaml = discoveredPaths.map { "  - \($0)" }.joined(separator: "\n")
        ...
        do {
            ...
            print("Created \(filePath) with paths: \(discoveredPaths.joined(separator: ", "))")
            return .success
        } catch {
            printError(error)
