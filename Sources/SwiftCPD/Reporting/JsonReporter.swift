...
extension JsonReporter {
    ...
    private func buildReport(from clones: [CloneGroup], result: AnalysisResult) -> JsonReport {
        ...
        let fileLines = buildFileLineCache(from: clones)
            ...
            JsonClone(
                fragments: clone.fragments.map { fragment in
                    JsonFragment(
                        ...
                        file: fragment.file,
                        preview: readPreview(for: fragment, fileLines: fileLines),
    ...
    private func buildFileLineCache(from clones: [CloneGroup]) -> [String: [String]] {
    ...
    private func readPreview(for fragment: CloneFragment, fileLines: [String: [String]]) -> String {
        guard
            let lines = fileLines[fragment.file]
        else {
            return ""
        }
