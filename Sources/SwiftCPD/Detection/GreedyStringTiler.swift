struct GreedyStringTiler: Sendable {
    ...
    func similarity(between tokensA: [Token], and tokensB: [Token]) -> Double {
        ...
        var state = GreedyTilingState(sizeA: tokensA.count, sizeB: tokensB.count)
...
extension GreedyStringTiler {
    ...
    private func computeTiles(tokensA: [Token], tokensB: [Token], state: inout GreedyTilingState) {
        var tileFound = true
    ...
    private func applyMatches(_ matches: [TileMatch], to state: inout GreedyTilingState) -> Bool {
        var applied = false
    ...
    private func canApply(_ match: TileMatch, state: GreedyTilingState) -> Bool {
        for offset in 0 ..< match.length {
